...
| `udea.kotlin-multiplatform-no-ios` | a runtime module that cannot have an iOS target yet, whose build script names why: `udea-core`, because Fleks publishes no iOS artifact (issue #215); `udea-gas`, which depends on `udea-core` (issue #204); `udea-replay`, which depends on `udea-core` (issue #206); `udea-net`, which depends on `udea-core` (issue #209) | `udea.kotlin-multiplatform` without the iOS targets; switching back to `udea.kotlin-multiplatform` re-enables them |
