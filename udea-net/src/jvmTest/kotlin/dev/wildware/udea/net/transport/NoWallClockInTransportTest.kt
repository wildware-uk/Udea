...
class NoWallClockInTransportTest {
    ...
    fun `the scan actually found source files to scan`() {
        ...
        // One file from each shipped source set the transports live in since issue #209, so a scan
        // that only walked `commonMain` - the old single `src/main`, renamed - goes red here.
        assertTrue(files.any { it.name == "UdpTransport.kt" }, "socketMain was not scanned")
        assertTrue(files.any { it.name == "WebSocketServerTransport.kt" }, "jvmMain was not scanned")
    }

    private fun scannedFiles(): List<File> {
        val projectDir = File(System.getProperty("udea.projectDir") ?: ".")
        val root = File(projectDir, "src")
        // Every shipped source set - `commonMain`, `socketMain`, `jvmMain`, `wasmJsMain` - and no
        // test one: the multiplatform layout splits what was one `src/main` by target (issue #209).
        val mainSourceSets = root.listFiles().orEmpty().filter { it.isDirectory && it.name.endsWith("Main") }
        return mainSourceSets
            .flatMap { sourceSet -> SCANNED_PACKAGES.map { File(sourceSet, "kotlin/dev/wildware/udea/net/$it") } }
