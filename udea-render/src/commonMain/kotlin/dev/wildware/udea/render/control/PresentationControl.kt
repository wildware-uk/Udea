 ...
 * `Deferred` the pipeline settles once a frame it is about to draw has been read; the camera and
...
public class PresentationControl(
    ...
    /**
     * Queues a capture and returns the `Deferred` the render thread settles.
     *
     ...
     *   out-of-range rectangle completes the `Deferred` exceptionally rather than reading past the
     *   end of the surface.
     ...
     *   render thread.
     * @return a `Deferred` completed with the frame, or completed exceptionally with
    ...
    public fun capture(
        ...
        afterTick: Long? = null,
    ): Deferred<CaptureResult> {
        val slot = pipeline.capture
            ?: return CompletableDeferred<CaptureResult>().apply {
