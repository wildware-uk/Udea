...
public class DiagToolset(
    ...
    /**
     * Heap as the runtime reports it, through [heapFigures].
     *
     * On a platform that reports no heap - Kotlin/Wasm - this refuses with [MEMORY_UNREPORTED]
     * rather than answering zeros, which an agent would read as a measurement (issue #208).
     */
    @AgentTool(
        name = "diag.memory",
    ...
    public fun memory(): AgentResult {
        val heap = heapFigures() ?: return AgentResult.failed(
            MEMORY_UNREPORTED,
            "this platform's runtime reports no heap figures, so diag.memory has nothing to " +
                "measure here; diag.system_timings still reports what the agent surface costs",
        )
        return AgentResult.ok {
            put("usedBytes", heap.usedBytes)
            put("committedBytes", heap.committedBytes)
            put("maxBytes", heap.maxBytes)
            put("processors", heap.processors)
        }
    }
    ...
    internal companion object {
        /** `diag.memory` on a runtime with no heap figures to report. */
        val MEMORY_UNREPORTED: AgentErrorKind = AgentErrorKind("memory_unreported")
    }
}
