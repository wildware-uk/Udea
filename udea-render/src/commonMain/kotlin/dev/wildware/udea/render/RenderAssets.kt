 ...
 * no authored value could override; and a `SpriteRegion` sitting in a component, which is a GL
 ...
 * @param x left edge in page pixels, y **top** edge in page pixels (the convention a packed image is written in).
...
/**
 * `AssetIndex` → `SpriteRegion`, as a flat array read.
 *
 ...
 * ## It owns nothing
 *
 * The page [SpriteTexture]s belong to whoever created them (in practice `RenderResources.own`, which
...
public class RenderAssets private constructor(
    private val regions: Array<SpriteRegion?>,
    ...
    public fun regionOrNull(index: AssetIndex): SpriteRegion? =
     ...
     */
    public fun region(index: AssetIndex): SpriteRegion = requireNotNull(regionOrNull(index)) {
    ...
    public companion object {
         ...
         */
        public fun of(pages: List<SpriteTexture>, regions: List<PackedRegion>): RenderAssets =
            build(regions) { region ->
                ...
                SpriteRegion(page, region.x, region.y, region.width, region.height)
            }
        ...
        private inline fun build(
            ...
            region: (PackedRegion) -> SpriteRegion?,
        ): RenderAssets {
            ...
            val table = arrayOfNulls<SpriteRegion>(size)
