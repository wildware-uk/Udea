...
/**
 * The window and capturable pass sizes a Kool backend is asked for.
 *
 * ## Why the render size is not the window size
 *
 * [renderWidth]/[renderHeight] size the offscreen pass the game is drawn into;
 * [windowWidth]/[windowHeight] size the window it is presented on. A capture reads the
 * pass, whose size is stated here and does not move when a human drags a window edge,
 ...
 * window-sized, every screenshot an agent diffed would carry the window manager's opinion
 ...
 * a sufficient one, and as of this wave **that criterion does not hold**. Wall-clock inputs
 * decide what a frame looks like, none of them a function of the tick:
 *
 ...
 * positions, playheads and camera. Serving a due request at `alpha = 0f` with
 * `frameSeconds = 0f` would fix the first input and only the first: the others are
...
public class WindowConfig(
    ...
    /** Width of the capturable pass, and so of every full-frame capture. */
    public val renderWidth: Int = DEFAULT_WIDTH,
    /** Height of the capturable pass, and so of every full-frame capture. */
     ...
     * never focused: leaving the unfocused rate at Kool's default would run the agent's host at
