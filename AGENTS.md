...
| `hollow:desktop` | Hollow's desktop launcher: `run` (a listen server and its local player, `-Plevel=<path>`), `runServer` (headless), `runClient` (`host` or `join`), `runShot`, `runPlayerShot` (the character walking, running a lap and stopping at a rock, one PNG per known tick), `runFoxShot` (a wave of foxes closing in on two players, drawn by either client of an in-process session, #251) and `runFightShot` (that wave fought off, with the HUD in every frame, and a non-zero exit when a frame has no HUD or nothing was fought, #252). Its `udeaHollowGlTest` soaks the HUD for sixteen seconds of real frames in each render mode. **A shot run twice is byte-identical**: the match seed is fixed, the network steps exactly one tick per frame, and capture happens at named ticks rather than off the wall clock - measured across three runs in two worktrees, two processes and one commit apart (#252). That is what makes a two-client comparison mean anything: "24 of 24 frames identical" is evidence that two machines agree only if a run means the same thing twice. JVM. No Android launcher in this epic |
