...
public class Json(initialCapacity: Int = DEFAULT_CAPACITY) {
    ...
    public fun value(value: Float) {
        ...
        // Ties round towards positive infinity, as `roundToLong` specifies. Finite here: NaN and
        // the infinities returned above, and `roundToLong` throws on NaN.
        val scaled = (value.toDouble() * FLOAT_SCALE).roundToLong()
        if (scaled == 0L) {
