...
internal class HandshakeRateLimiter(
    ...
    private fun hostKey(address: InetSocketAddress): Int =
        address.resolveAddress()?.contentHashCode() ?: address.hostname.hashCode()

    /**
     * Spreads a key across the table.
     *
     * The content hash of an address's bytes moves by one when its last octet does, so
     * consecutive addresses in a subnet land in consecutive slots and a `/24` sweep would walk one
     * probe span. The finalizer breaks that up.
     */
