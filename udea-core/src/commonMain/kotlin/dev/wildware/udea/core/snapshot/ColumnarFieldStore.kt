...
public class ColumnarFieldStore(
    ...
    public fun ensureSlots(required: Int) {
        ...
        val grownInts = regrow(ints, ColumnGroup.Ints, capacity, ::IntArray) { to, at, from, until ->
            copyInto(to, at, from, until)
        }
        val grownLongs = regrow(longs, ColumnGroup.Longs, capacity, ::LongArray) { to, at, from, until ->
            copyInto(to, at, from, until)
        }
        val grownFloats = regrow(floats, ColumnGroup.Floats, capacity, ::FloatArray) { to, at, from, until ->
            copyInto(to, at, from, until)
        }
        val grownObjects = regrow(objects, ColumnGroup.Objects, capacity, ::arrayOfNulls) { to, at, from, until ->
            copyInto(to, at, from, until)
        }
    ...
    override fun setObject(slot: Int, field: Int, value: Any?) {
        ...
        require(hasStableHashCode(value)) {
            "${schema.typeName}.${schema.nameOf(field)} was given a ${value!!::class.runtimeName}, " +
    ...
    private inline fun <A> regrow(
        ...
        capacity: Int,
        allocate: (Int) -> A,
        copyRange: A.(destination: A, destinationOffset: Int, startIndex: Int, endIndex: Int) -> Unit,
    ): A where A : Any {
