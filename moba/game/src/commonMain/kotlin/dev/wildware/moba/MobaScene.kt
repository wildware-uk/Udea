...
import dev.wildware.moba.render.MobaAtlas
...
public class MobaScene private constructor(
     ...
     * `build` asked for the framing and `MobaLaunch.follow` asked for the player. So frame one
 ...
 * `orcScale` in `moba/game/assets/character/orc.udea.kts` changes the size of every orc in the very
...
internal class CharacterRenderSystem(
     ...
     * "startup".
     */
    private val framesBySheet: Map<AssetId, Array<SpriteRegion>> =
        MobaAtlas.frames(resources)
    ...
    private fun World.draw(entity: Entity, batch: SpriteBatch2D, now: Long, tickRate: Int) {
        ...
        // `flipX` on the draw rather than a flipped region: the regions are shared by every
        // entity drawing that sheet, so flipping one in place would mirror the whole roster for
        // the rest of the frame. The LibGDX version got the same effect by passing a negative
        // width, which described the same rectangle; this says what it means.
        batch.draw(
            region = frame,
            x = position.x - width / 2f,
            y = position.y - height / 2f,
            width = width,
            height = height,
            flipX = view.flipX,
        )
    ...
    internal companion object {
        ...
        val PLAYER_COLOUR: Rgba = WorldMarkers.PLAYER_COLOUR
