 ...
 *
 * A drawing system needs a batch, and a batch needs a render context, and a GL context does not
...
public class RenderResources internal constructor(
     ...
     *
     * ```
     * private val font = resources.own(BitmapFont2D.builtIn())
     * ```
     */
    public fun <T : RenderResource> own(resource: T): T {
        extra += resource
        return resource
    }
...
/**
 * What an [OverlaySystem]'s constructor is handed: the screen batch, the never-captured
 ...
 *
 * The [batch] is **not** the one [RenderResources] carries. It is a second batch, drawn only in
 * the window's scene, and the capture reads a texture that batch never reaches. So the exclusion
 * holds at this level too: nothing an overlay holds leads to a capturable pixel.
 */
public class OverlayResources internal constructor(
    /** The batch that reaches the window, and never the capturable pass. */
    public val batch: SpriteBatch2D,
    /**
