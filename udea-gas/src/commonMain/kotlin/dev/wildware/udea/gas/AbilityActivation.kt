...
public class AbilityActivation(
    ...
    public fun effectiveCooldownTicks(def: AbilityDef, attributes: Attributes): Int {
        ...
        val basisPoints = roundHalfUp(percent * BASIS_POINTS_PER_PERCENT)
...
/**
 * [value] rounded half up, to an `Int`: the rule `java.lang.Math.round(Float)` applies, spelled so
 * every target computes it.
 *
 * The cooldown reduction was `Math.round` before this module was multiplatform (issue #204), and a
 * cooldown is authoritative state, so the port has to change nothing the JVM server computes.
