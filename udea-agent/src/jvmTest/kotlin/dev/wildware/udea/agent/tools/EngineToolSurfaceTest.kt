...
class EngineToolSurfaceTest {
    ...
    fun `every generated engine tool is in a module a host can register`() {
        // The common KSP run's manifest (issue #208): every engine toolset is common code.
        val manifest = File("build/generated/ksp/metadata/commonMain/resources/udea/UdeaAgent-agent-tools.json")
        assertTrue(manifest.isFile, "no generated manifest at ${manifest.absolutePath}; run kspCommonMainKotlinMetadata")
        ...
        val registered = EngineToolModules.ALL
    ...
    /**
     * The same gap for `AssetToolModule`, whose toolset is JVM-only and so is generated by the
     * JVM KSP run, which writes tool objects and no manifest (issue #208). The generated objects
     * are then the other side of the hand-written list: one per `@AgentTool`, with no chance to
     * forget one.
     */
    @Test
    fun `every generated assets tool is in the module a host can register`() {
        val generated = File("build/generated/ksp/jvm/jvmMain/kotlin/dev/wildware/udea/agent/assets")
        assertTrue(generated.isDirectory, "no generated assets tools at ${generated.absolutePath}; run kspKotlinJvm")

        val objects = generated.listFiles { file -> file.name.endsWith("Tool.kt") }.orEmpty()
            .map { it.name.removeSuffix(".kt") }
            .toSortedSet()
        val registered = AssetToolModule.Assets.tools.mapNotNull { it::class.simpleName }.toSortedSet()

        assertTrue(objects.isNotEmpty(), "the JVM KSP run generated no assets tool")
        assertEquals(
            objects,
            registered,
            "the generated assets tools and AssetToolModule disagree; a tool in one and not the " +
                "other is either generated and unreachable, or listed and never generated",
        )
    }
