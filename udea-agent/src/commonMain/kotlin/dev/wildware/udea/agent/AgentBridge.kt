...
public class AgentBridge(
    ...
    /**
     * Written by whichever thread serves the agent and drained by the simulation thread, so it is
     * only touched under [commandsLock]: common code has no lock-free queue, and a lock held for
     * one `addLast` or `removeFirst` is not contended at 256 commands.
     */
    private val commands = ArrayDeque<AgentCommand>()

    private val commandsLock = SynchronizedObject()

    /**
     * The cap is enforced against a counter taken before [commands] is touched, so a submission
     * that would overflow is refused without ever entering the queue.
     */
     ...
     */
    public val frame: Long get() = frames.load()
     ...
     */
    public val tick: Long get() = ticks.load()
    ...
    public fun submit(command: AgentCommand): AgentSubmission {
        ...
        val depth = queued.incrementAndFetch()
    ...
    public fun completedCommandId(): Long = completed.load()
    ...
    public fun drain(into: MutableList<AgentCommand>): Int {
        ...
        while (remaining > 0) {
            val command = synchronized(commandsLock) { commands.removeFirstOrNull() } ?: break
            queued.decrementAndFetch()
            into.add(command)
            drained++
            remaining--
     ...
     */
    public fun readSinceLastPublish(): Boolean = readSincePublish.load()

    /** Advances [frame] by one and returns the new value. */
    public fun advanceFrame(): Long = frames.incrementAndFetch()
    ...
    override fun toString(): String =
        "AgentBridge(frame=$frame, tick=$tick, pending=$pendingCommands, completed=${completed.load()})"
...
internal class CommandResultRing(private val capacity: Int) {
    ...
    /** The monitor every read and write below holds: atomicfu's, so it exists in common code. */
