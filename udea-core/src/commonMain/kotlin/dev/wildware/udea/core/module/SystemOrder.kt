...
internal object SystemOrder {
    ...
    fun sort(nodeCount: Int, edges: List<OrderEdge>, describe: (Int) -> String): List<Int> {
        ...
        val ready = IndexMinHeap(nodeCount)
...
/**
 * The ready set of [SystemOrder.sort]: a binary min-heap of node indices.
 *
 * The sort's tie-break is "lowest registration index first", and a min-heap is what gives that
 * in `O(log n)` per node. It was `java.util.PriorityQueue<Int>` until `udea-core` became
 * multiplatform (issue #203); node indices are distinct, so any correct min-heap pops them in
 * exactly the same order, and `SystemOrderTest` pins that order.
 *
 * @param capacity the node count. Each node enters the heap at most once, when its in-degree
 *   reaches zero, so the heap never holds more.
 */
private class IndexMinHeap(capacity: Int) {
    ...
    fun add(node: Int) {
        check(size < heap.size) { "IndexMinHeap is full at ${heap.size}; a node was made ready twice" }
        var child = size++
        heap[child] = node
        while (child > 0) {
    ...
    /** Removes and returns the lowest index. */
    fun poll(): Int {
        check(size > 0) { "IndexMinHeap is empty" }
        val lowest = heap[0]
        heap[0] = heap[--size]
        var parent = 0
        while (true) {
