...
back. `udea-gas` is on the same convention for the same reason, through `udea-core` (issue #204).
`udea-net` is too (issue #209): its UDP transport runs on `jvm` and `android` through a shared
`socketMain` source set, and `wasmJs` has the WebSocket client only.
