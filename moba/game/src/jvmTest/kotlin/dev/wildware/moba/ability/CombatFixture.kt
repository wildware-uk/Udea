...
internal class CombatFixture(autopilot: Boolean = true) {
    ...
    private val definition: UdeaGameDef =
        UdeaGameDef(
            ...
            modules = listOf(
                module,
                // No physics module. `MobaGame.definition()` has never installed one, so
                // `ctx.physics` in the shipped game is `NoOpPhysicsWorld`, and a fixture that
                // installed Box2D was the fixture testing a different simulation from the one
                // that ships - the reverse of what the comment here used to claim. Box2D left
                // with LibGDX (spec D4) and nothing in `moba`'s production source calls
                // `PhysicsWorld.overlap`; `ProjectileSystem` scans combatants itself.
