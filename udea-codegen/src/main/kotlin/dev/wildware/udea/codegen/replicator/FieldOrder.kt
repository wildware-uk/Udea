 ...
 *
 * A field's bit index is its position in the component's annotated properties **sorted by
 * property name**, ascending, using the ordinary lexicographic ordering of the name's UTF-16
 * code units. `@Net` and `@Sim` properties share one index space: `fieldNames[i]`, `FieldMask`
 * bit `i` and `FieldStore` field `i` are all the same `i`, so a `@Sim` field cannot be
 * "skipped" when assigning network bits without breaking that alignment.
 *
...
internal object FieldOrder {
    ...
    /**
     * The `FIELD_…` constant name generated for a property, e.g. `lastGroundedTick` becomes
     * `FIELD_LAST_GROUNDED_TICK`.
     *
     * Two different property names can screaming-snake to the same constant (`fooBar` and
     * `foo_bar`), so [constantNames] — not this function — is what generated code uses.
     */
    fun constantName(propertyName: String): String {
    ...
    /**
     * Constant names for [propertyNames], in the same order, made unique.
     *
