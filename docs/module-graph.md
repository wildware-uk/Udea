...
| `udea-nav` | `udea.kotlin-multiplatform` | Ground navigation for many units (issue #264): a nav grid rebuilt from the `NavObstacle` footprints standing in the world, integer-cost A* for one unit, one shared flow field per goal cell for a group, and separation so a crowd does not stand in itself. Headless and `udea-core`-only, so it runs on a dedicated server and inside a replay; the `nav.*` toolset is `jvmMain` over `compileOnly(udea-agent)`, which keeps the agent surface off a shipped runtime classpath the way `udea-replay` does | new | `udea-core` (api); `udea-agent` (`compileOnly`, `jvmMain` only) | *(nothing yet - a game adds `NavModule`)* |
