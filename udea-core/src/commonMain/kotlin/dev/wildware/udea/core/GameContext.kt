...
public class GameContext internal constructor(
    ...
    public fun <T : Any> getOrNull(key: ServiceKey<T>): T? {
        ...
        if (!key.type.isInstance(value)) {
            throw ClassCastException("${key.name} holds a ${value::class.simpleName}, not a ${key.type.simpleName}")
        }
        @Suppress("UNCHECKED_CAST") // Checked on the line above; `KClass.cast` is JVM-only reflection.
        return value as T
    }
