...
class UdpTransportTest {
    ...
    fun `a full server denies rather than dropping the request on the floor`() {
        UdpPair(serverConfig = UdpConfig(maxClients = 1)).use { pair ->
            ...
            second.use {
                // The server keeps sending the first client keep-alives, which `second` never sees, so
                // what `second` waits for is the server's total less what went to the first client.
                val first = pair.server.stats(PeerId.client(1))
                val notForSecond = pair.server.datagramsTransmitted - first.packetsSent
                repeat(UdpPair.DEFAULT_LIMIT) {
                    if (second.failure != null) return@repeat
                    pair.clock.advance()
                    pair.server.flush()
                    second.flush()
                    settle(pair.server, pair.client.datagramsTransmitted + second.datagramsTransmitted)
                    pair.server.poll(pair.serverSink)
                    settle(second, pair.server.datagramsTransmitted - first.packetsSent - notForSecond)
    ...
    fun `sequence numbers wrap and delivery continues across 200000 real datagrams`() {
        UdpPair().use { pair ->
            ...
            while (sent < TOTAL_PACKETS) {
                ...
                pair.server.flush()
                pair.client.flush()
                // Loopback may drop at this rate, which is what this test tolerates below.
                settleWithLoss(pair.server, pair.client.datagramsTransmitted)
