 ...
 *
 * A class name is `runtimeName`: the binary name `Outer$Inner` on the JVM and Android, which is
 * what a game passes as `SomeExec::class.java.name`, and the qualified name elsewhere. Its KDoc
 * says when the two spellings could order a registry differently.
 */
public class AbilityExecRegistry private constructor(
    ...
    public fun idOf(exec: AbilityExec): AbilityExecId {
        val name = exec::class.runtimeName
    ...
    public companion object {
        ...
        public fun of(execs: List<AbilityExec>): AbilityExecRegistry {
            val sorted = execs.sortedBy { it::class.runtimeName }
            val names = Array(sorted.size) { sorted[it]::class.runtimeName }
