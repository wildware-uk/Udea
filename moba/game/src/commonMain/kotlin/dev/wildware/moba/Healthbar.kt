...
public class HealthbarRenderSystem(
     ...
     *
     * `SpriteBatch2D.fill` and not a one-texel white texture this class uploads and stretches.
     * The batch owns exactly one white texel for this - it is what `fill` records against - so the
     * bars share it with every other solid rectangle in the frame and land in the same run instead
     * of starting a new one.
     */
    private fun rail(left: Float, bottom: Float, filled: Float, colour: Rgba) {
        batch.fill(left, bottom, WIDTH, HEIGHT, BACKGROUND)
        if (filled <= 0f) return
        batch.fill(left, bottom, WIDTH * filled, HEIGHT, colour)
    }
    ...
    public companion object {
        ...
        public fun colourOf(team: Int): Rgba = when (team) {
            Team.ORC -> ORC_COLOUR
        ...
        internal val BACKGROUND: Rgba = Rgba.of(0.10f, 0.03f, 0.03f, 0.85f)

        internal val ORC_COLOUR: Rgba = Rgba.of(0.85f, 0.35f, 0.18f, 1f)

        internal val SOLDIER_COLOUR: Rgba = Rgba.of(0.32f, 0.68f, 0.95f, 1f)

        internal val UNDEAD_COLOUR: Rgba = Rgba.of(0.62f, 0.85f, 0.45f, 1f)

        internal val NEUTRAL_COLOUR: Rgba = Rgba.of(0.7f, 0.7f, 0.7f, 1f)

        /** The mana rail. Blue, and the same blue in every team's colours. */
        internal val MANA_COLOUR: Rgba = Rgba.of(0.30f, 0.36f, 0.90f, 1f)
         ...
         */
        internal val PLAYER_COLOUR: Rgba = WorldMarkers.PLAYER_COLOUR
    }
}
