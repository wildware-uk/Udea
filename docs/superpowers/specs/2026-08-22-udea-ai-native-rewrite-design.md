...
|---|---|---|
| `udea-annotations` | Zero-dependency leaf: `@Net`, `@Sim`, `@Q`, `@Replicated`, `@AgentTool`, `@Arg`, `@AgentState` | Two conflicting `UdeaNetworked` declarations on one classpath |
...
| `udea-audio` | Cue-driven sound: the drain that empties `GameContext.cues`, the routing table, distance attenuation, stereo pan, pitch variance and the per-frame voice cap, behind an `AudioDevice` SPI with no LibGDX type in it. Headless-clean (`AudioDevice.Silent` is a shipped implementation, not a stub); the backend that opens a `Sound` lives in the game. Seconds and a wall-seeded generator are allowed here (§5) | `common/.../ecs/system/SoundSystem.kt`, which read `gameScreen.camera` off a file-level global from inside a Fleks system and called `play` on a `Sound` held by an *asset value* |
