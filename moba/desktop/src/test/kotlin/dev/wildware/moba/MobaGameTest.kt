...
class MobaGameTest {
    ...
    fun `an unknown render mode is refused, not defaulted`() {
        assertEquals(
            ...
            MobaLaunch.modeFromProperties(RenderMode.Offscreen) { null },
        )
        assertEquals(
            RenderMode.Windowed,
            MobaLaunch.modeFromProperties(RenderMode.Offscreen) { "Windowed" },
        )
        val failure = assertFailsWith<IllegalArgumentException> {
            MobaLaunch.modeFromProperties(RenderMode.Offscreen) { "Offscren" }
        }
