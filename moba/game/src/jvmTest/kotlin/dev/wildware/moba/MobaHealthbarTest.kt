...
class MobaHealthbarTest {
     ...
     *
     * `assertEquals` and not `assertSame`: a colour is a `Rgba` now, a value class over a packed
     * `0xRRGGBBAA` int, and identity of a boxed value class says nothing a caller can rely on.
     * What the test is about - that an unknown team resolves to the neutral colour rather than
     * throwing - is unchanged, and equality is the stronger claim of the two here anyway.
     */
    @Test
    fun `an unknown team is neutral, not an exception`() {
        assertEquals(HealthbarRenderSystem.NEUTRAL_COLOUR, HealthbarRenderSystem.colourOf(Team.NONE))
        assertEquals(HealthbarRenderSystem.NEUTRAL_COLOUR, HealthbarRenderSystem.colourOf(99))
    }
