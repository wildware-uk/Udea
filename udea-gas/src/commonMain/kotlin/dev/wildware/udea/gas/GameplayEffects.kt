...
public class GameplayEffects(
    ...
    public fun add(
        ...
        setByCallerTags.fill(GameplayTag.NONE.id, base, base + SET_BY_CALLER_SLOTS)
        setByCallerValues.fill(0f, base, base + SET_BY_CALLER_SLOTS)
        count++
        return slot
    }
    ...
    public fun removeAt(slot: Int) {
        ...
        if (moved > 0) {
            ...
            nextPeriodTicks.copyInto(nextPeriodTicks, index, index + 1, count)
            periodTicks.copyInto(periodTicks, index, index + 1, count)
            stacks.copyInto(stacks, index, index + 1, count)
            sources.copyInto(sources, index, index + 1, count)
            setByCallerTags.copyInto(
                setByCallerTags,
                index * SET_BY_CALLER_SLOTS,
                (index + 1) * SET_BY_CALLER_SLOTS,
                count * SET_BY_CALLER_SLOTS,
            )
            setByCallerValues.copyInto(
                setByCallerValues,
                index * SET_BY_CALLER_SLOTS,
                (index + 1) * SET_BY_CALLER_SLOTS,
                count * SET_BY_CALLER_SLOTS,
