...
class DebugDrawSystem : IteratingSystem(
    family { all(Transform, DebugComponent) }
) {
    ...
    override fun onTickEntity(entity: Entity) {
        ...
        val debug = entity[DebugComponent]

        val position = camera.project(Vector3(transform.position, 0f))
        ...
        if (debug.drawStats) {
            val stats = entity[AbilitiesComponent]
            size32Font.draw(spriteBatch, "Stats: ${stats.attributeSet}", position.x, position.y + 150F)
        }

        if (debug.debugPhysics && entity.has(RigidBodyComponent)) {
            val rb = entity[RigidBodyComponent]
