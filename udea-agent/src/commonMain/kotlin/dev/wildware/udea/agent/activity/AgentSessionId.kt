 ...
 * overlay printing a label), so every read and write holds one lock. Nothing here is on the
 * per-tick path: interning happens once per distinct label, and the overlay looks a label up
 * only when the panel is re-formatted, which is on change and not per frame.
 */
public class AgentSessions(
    ...
    /** Guards [ids], [labels] and [next]: atomicfu's lock, so it exists in common code. */
    ...
    public val size: Int get() = synchronized(lock) { labels.size }
    ...
    public fun intern(label: String): AgentSessionId {
        ...
        // The capacity check and the id assignment are one step under the lock: an id handed
        // out and then given back on overflow would leave a hole in the dense sequence the
        // palette indexes.
        synchronized(lock) {
    ...
    public fun label(id: AgentSessionId): String = synchronized(lock) { labels[id.raw] } ?: UNKNOWN_LABEL
