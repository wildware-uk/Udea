...
public class ConnectionSecret(key: ByteArray) {
    ...
    /**
     * HMAC-SHA-256 under [key], through `cryptography-kotlin` rather than `javax.crypto` (issue
     * #209), and one sign function reused for every token rather than one per call.
     */
    private val mac: SignFunction = CryptographyProvider.JDK.get(HMAC)
        .keyDecoder(SHA256)
    ...
    /** Reused MAC output. `SignFunction.signIntoByteArray` writes into it rather than allocating. */
