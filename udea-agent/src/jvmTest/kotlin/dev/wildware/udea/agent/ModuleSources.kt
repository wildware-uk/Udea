...
internal object ModuleSources {
    ...
    /**
     * Every `.kt` file in every main source set - `commonMain`, `jvmMain` and each shared or
     * platform one (issue #208) - in path order.
     *
     * Every one rather than `commonMain` alone: a rule about what this module's code may import
     * is broken just as badly by a `jvmMain` file, which is exactly where a JVM-only import can
     * compile.
     */
