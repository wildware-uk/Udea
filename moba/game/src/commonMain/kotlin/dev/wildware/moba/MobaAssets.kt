...
public object MobaAssets {
    ...
    private fun open(): Bundle = BundleReader.open(readBundleBytes())
}

/**
 * The bytes of [MobaAssets.RESOURCE], however this platform reaches them.
 *
 * An `expect` rather than a `ClassLoader` call in common code, because a class loader is a JVM
 * idea. On the JVM and on Android it is the classpath resource the packaging step put there; on
 * the web (issue #226) it is an HTTP fetch of the same file, which is what spec section 4 means by
 * "the web build fetches the pak over HTTP".
 *
 * Read whole and handed over rather than opened as a seekable `BundleSource`: the bundle is inside
 * a jar on a packaged run and inside an APK on a phone, so there is no file to seek in, and this
 * game's is 23KB.
 *
 * @throws IllegalStateException when the resource is absent, naming the task that writes it. There
 *   is deliberately no fallback - see [MobaAssets].
 */
