 ...
 *
 * Regenerate deliberately with `./gradlew :udea-core:jvmTest -Dupdate.goldens=true`, and treat a
...
class CoreModuleManifestGoldenTest {
    ...
    fun `the CoreModule manifest matches the golden file`() {
        ...
        assertTrue(
            ...
            "missing golden ${golden.path}; regenerate with " +
                "./gradlew :udea-core:jvmTest -Dupdate.goldens=true",
        )
        assertEquals(
            golden.readText().replace("\r\n", "\n"),
            rendered,
            "CoreModule's system order changed; regenerate with " +
                "./gradlew :udea-core:jvmTest -Dupdate.goldens=true if that was deliberate",
        )
    }
    ...
    private companion object {
        const val GOLDEN_PATH = "src/jvmTest/resources/golden/core-module-systems.txt"
    }
}
