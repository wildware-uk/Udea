...
//                    println("[SERVER] received $obj")
    ...
    override fun onTick() {
        inputQueue.processAndRemoveEach { packet ->
            when (packet) {
                is EntityUpdate -> world.processEntityUpdate(packet, Client)
        ...
        newEntities.processAndRemoveEach {
            server.sendToAllTCP(it.toEntityCreate(world, Server))
        }
    ...
    override fun onTickEntity(entity: Entity) {
        val snapshot = entity.toEntityUpdate(world, Server)
    ...
    fun World.getEntityCreates(): List<EntityCreate> {
        return family.map {
            it.toEntityCreate(world, Server)
        }
    }
}
