...
public class GameLoop(
    ...
    /**
     * The thread inside `sim.step()` right now, as its [currentThreadToken], or `null` between
     * ticks.
     *
     ...
     */
    @Volatile
    private var tickingThread: Any? = null
    ...
    public fun tickIfRunning(): Boolean {
        ...
        tickingThread = currentThreadToken()
        try {
            if (paused) return false
            sim.step()
        } finally {
    ...
    public fun pauseAtBoundary() {
        paused = true
        val self = currentThreadToken()
        while (true) {
            val owner = tickingThread ?: return
            if (owner === self) return
            yieldToTickingThread()
        }
    }
