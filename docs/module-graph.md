...
**An id says whether the plugin is published.** Gradle publishes a *plugin marker* for every
plugin - a POM whose group is the plugin id and whose artifact is `<id>.gradle.plugin` - so a
plugin id is a Maven group, and Sonatype authorises a publisher per namespace. Ours is
`dev.wildware`; a PUT to any other comes back 403, which is what took the first real run of
`.github/workflows/release.yml` down (issue #265).

- **`dev.wildware.udea.*`** is published. A game in its own repository applies it by id, so its
  marker has to resolve from a repository. `templates/new-game/settings.gradle.kts` is the list.
- **`udea.*`** is internal to this build. One precompiled script plugin applying another gets it
  off the jar's own classpath, so no marker is involved and none is published. Making one public
  is a rename, not a mention.

`PluginNamespaceTest` holds the two apart in source, and `scripts/outside-game-proof.sh` reads
the coordinates back out of a repository a publish actually wrote.
...
| `dev.wildware.udea.kotlin-library` | every JVM runtime module, `moba:desktop` and `hollow:desktop` | Kotlin JVM plus `udea.kotlin-base`, kotlin.test on JUnit 5. **No GL.** |
| `dev.wildware.udea.kotlin-multiplatform` | runtime modules ported to KMP (issue #201) | Kotlin Multiplatform on `jvm`, `android` (AGP's KMP library plugin), `wasmJs` (Node), `iosArm64`, `iosSimulatorArm64`, plus `udea.kotlin-base`; kotlin.test in `commonTest`, JUnit 5 on `jvmTest` |
| `udea.kotlin-multiplatform-no-ios` | a runtime module that cannot have an iOS target yet, whose build script names why: `udea-net`, whose `webSocketEngine` has no native `actual` (issue #209); `udea-agent`, whose `enumConstantsOf` and `heapFigures` have none (issue #208). `udea-core` left it when Fleks was vendored (issue #215) | `dev.wildware.udea.kotlin-multiplatform` without the iOS targets; switching back to `dev.wildware.udea.kotlin-multiplatform` re-enables them |
| `dev.wildware.udea.kotlin-multiplatform-render` | `udea-render` (spec D2, issue #211), and `moba:game` and `hollow:game`, which build for every target `udea-render` has (issues #212, #249) | `udea.kotlin-multiplatform-no-ios`, applied rather than copied |
| `dev.wildware.udea.compose-ui` | applied **beside** a Kotlin convention, in either order, by a project that writes composables: `udea-render`, `udea-editor`, `moba:game`, and a game's own interface (issue #275) | the Compose compiler at the catalog's Kotlin version, and nothing else; the toolkit a screen is written in, `composegl-ui`, comes through `udea-render` |
...
| `udea.kotlin-build-tool` | `udea-codegen`, `udea-compiler-plugin`, `udea-assets-compiler` | `dev.wildware.udea.kotlin-library` plus the exact-Kotlin-version pin (spec §7), checked at configuration time |
| `udea.gradle-plugin` | `udea-gradle` | `dev.wildware.udea.kotlin-library` plus `compileOnly(gradleApi())` and TestKit for tests |
...
| `udea-core` | `dev.wildware.udea.kotlin-multiplatform` | Headless kernel: `Simulation`, `SimBarrier`, `NetId`, `Tick`, snapshot ring, `Replicator`. **No GL on the compile classpath** | `UdeaGameManager`/`GameScreen`, the globals, `common/.../properties.kt`, `reflection.kt` | `udea-annotations` (api), `udea-fleks` (api), `udea-assets` (api, issue #270: `Drawn` names a model with a `Ref<Model>`, and a headless simulation is what spawns the parts that carry one), kotlinx-serialization-core (api) and -cbor for level files (issue #191), atomicfu for the `SimBarrier` inbox lock (issue #203) | `udea-gas`, `udea-net`, `udea-physics2d`, `udea-render`, `udea-agent`, `moba` |
| `udea-assets` | `dev.wildware.udea.kotlin-multiplatform` | Runtime asset model + `.udeapak` reader | `common/assets/*`, the `Assets` global | `udea-annotations` (api), `udea-diagnostics` | `udea-core`, `udea-assets-compiler`, `udea-audio`, `udea-render`, `moba` |
