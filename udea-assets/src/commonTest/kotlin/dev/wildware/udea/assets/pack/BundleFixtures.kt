...
internal object BundleFixtures {
    ...
    fun graphSection(assets: List<Triple<String, String, Map<String, String>>>): ByteArray {
        ...
        assets.forEach { (id, kind, fields) ->
            ...
            out.int(fields.size)
            fields.entries.sortedBy { it.key }.forEach { (name, value) ->
    ...
    fun emptyAtlasSection(): ByteArray = ByteBuilder().apply {
        int(0) // strings
        int(0) // pages
        int(0) // regions
        int(0) // sheets
    ...
    fun bundle(
        ...
        val tocSize = sections.sumOf { 1 + 1 + 2 + it.second.encodeToByteArray().size + 8 + 4 }
        ...
        header.int(version)
        header.int(0)
        header.write(Sha256.digest(bodyBytes))
    ...
    /**
     * A growable byte buffer: the one piece of `java.io.ByteArrayOutputStream` these fixtures used,
     * written out so they compile for every target.
     */
    private class ByteBuilder {
        ...
        fun write(values: ByteArray) = values.forEach { write(it.toInt()) }
        ...
        private companion object {
            /** Grown by doubling; the size of a small fixture section, so most never grow. */
