...
import dev.wildware.moba.render.JdkRandom
import dev.wildware.moba.render.PixelCanvas
import dev.wildware.moba.render.floorMod
 ...
 * *computed*: a seamless 256x256 image of banded grass with trodden-earth patches, blades and
 * stones, built once at construction from a fixed seed and uploaded once.
 *
 * The seed is a constant and the generator is [JdkRandom], not
 * [dev.wildware.udea.core.RngService]: this runs on the render thread, once, before the first
 * frame, and never again. It is not simulation, it takes no draw from a seeded stream, and a
 * `time.rewind` cannot change it - which is the property that matters, because a background that
 * differed between two captures of the same paused world would put noise into every
 * `render.compare_artifacts` diff in the game. [JdkRandom] is `java.util.Random`'s specified
 * stream written out in common code, so the tile is the same grass the LibGDX build generated and
 * the port's parity comparison measures the renderer rather than the choice of generator.
 *
 * ## One run, no per-frame allocation
 *
 * [render] computes the visible world rectangle from the camera, snaps it out to whole tiles, and
 * issues one `batch.draw` per tile - about thirty-five of them at this game's framing, all from
 * one texture, so `SpriteBatch2D` records them as a single run and the frame is still one draw
 * call. The [SpriteRegion] is built once, so a frame allocates nothing here.
 */
internal class BackgroundRenderSystem(
    ...
    /** The one tile, uploaded once. */
    private val texture: SpriteTexture = resources.own(buildTile().toTexture("moba-ground-tile"))

    /**
     * The whole of [texture], built once.
     *
     * A field and not a local for the reason the LibGDX version had one: this is the only path in
     * this system that runs every frame, and a `SpriteRegion` per frame is a per-frame allocation.
     * It is no longer *re-pointed* - see [render] for why the u/v span went away.
     */
    private val region = SpriteRegion(texture)

    /** Tiles drawn by the most recent frame. What `MobaSceneTest` counts instead of pixels. */
    ...
    override fun render(target: OffscreenTarget, alpha: Float) {
        ...
        val halfWidth = viewport.worldWidth * view.zoom * 0.5f + TILE_WORLD
        val halfHeight = viewport.worldHeight * view.zoom * 0.5f + TILE_WORLD
        val left = floor((view.position.x - halfWidth) / TILE_WORLD).toInt()
        val bottom = floor((view.position.y - halfHeight) / TILE_WORLD).toInt()
        val right = ceil((view.position.x + halfWidth) / TILE_WORLD).toInt()
        val top = ceil((view.position.y + halfHeight) / TILE_WORLD).toInt()
        drawnCount = 0
        batch.begin(camera.projection)
        try {
            // One quad per tile, and not one quad with a u/v span wider than 1.
            //
            // The LibGDX version asked the texture for `TextureWrap.Repeat` and let the GPU tile a
            // single quad. `SpriteTexture` samples `clamped()` for every texture it makes - which
            // is what stops a region at the edge of an atlas page bleeding into the opposite edge,
            // and is not negotiable per texture - so a u/v of 5 would smear the tile's last column
            // across the field instead of repeating it. At this game's framing the visible area is
            // about seven tiles by five, all from one texture, so it is one run and one draw call
            // either way; what it costs is thirty-five instances in the batch's arrays, which are
            // preallocated.
            for (tileY in bottom until top) {
                for (tileX in left until right) {
                    batch.draw(
                        region,
                        tileX * TILE_WORLD,
                        tileY * TILE_WORLD,
    ...
    internal companion object {
         ...
         * through `floorMod`, so the right edge meets the left with no seam and there is no
         * wrapped blur pass to get wrong.
         *
         * Composed in an `IntArray` and written out afterwards rather than one call per texel: at
         * 256 square that is 65,536 of them, on the render thread, before the first frame - which
         * is start-up time `udeaBenchStartup` gates.
         */
        private fun buildTile(): PixelCanvas {
            ...
            repeat(BLADES) {
                ...
                for (step in 0 until height) {
                    texels[floorMod(y + step, size) * size + x] = blade
                }
            }
            ...
            repeat(STONES) {
                ...
                texels[y * size + floorMod(x + 1, size)] = stone
                texels[floorMod(y + 1, size) * size + x] = stone
            }
            val canvas = PixelCanvas(size, size)
            for (y in 0 until size) {
                for (x in 0 until size) {
                    val texel = texels[y * size + x]
                    canvas.set(
                        x, y,
                        (texel ushr 24) and 0xFF,
                        (texel ushr 16) and 0xFF,
                        (texel ushr 8) and 0xFF,
                    )
                }
            }
            // Nothing asks for a filter or a wrap mode: `SpriteTexture` samples `nearest()` and
            // `clamped()` for every texture it makes. Nearest is what this always wanted - the
            // sprites over it are pixel art, and a linearly filtered ground under them looks like
            // a different game - and clamped is why `render` draws a quad per tile.
            return canvas
        }
        ...
        private fun stampPatch(texels: IntArray, cx: Int, cy: Int, radius: Int) {
            ...
            for (dy in -radius..radius) {
                for (dx in -radius..radius) {
                    ...
                    val index = floorMod(cy + dy, size) * size + floorMod(cx + dx, size)
        ...
        /** Opaque RGBA8888: the byte order the composition array holds before it reaches the canvas. */
         ...
         *
         * Deliberately not [JdkRandom]: a hash is a *function of the coordinate*, so the
