...
/**
 * `dev.wildware.udea.kotlin-multiplatform` and `dev.wildware.udea.kotlin-multiplatform-render`,
 * applied to a real build
...
class KotlinMultiplatformConventionTest {
    ...
    fun `the runtime convention targets JVM, Android, Wasm and both iOS ARM targets`(@TempDir root: File) {
        val result = multiplatformFixture(root, "dev.wildware.udea.kotlin-multiplatform")
            .build(":sample:printTargets")
    ...
    fun `the render convention is JVM and Android - Kool has neither iOS nor a Wasm artifact`(@TempDir root: File) {
        ...
        val result = multiplatformFixture(root, "dev.wildware.udea.kotlin-multiplatform-render")
            .build(":sample:printTargets")
