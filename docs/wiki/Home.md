...
- **The build writes the boring code.** A KSP processor reads annotations such as `@Replicated`, `@Net` and `@Sim` and generates one `Replicator<T>` per component. That one class serves networking, rewind, save files and the agent's field access. See [ECS and Components](ECS-and-Components) and [Replication and Networking](Replication-and-Networking).
