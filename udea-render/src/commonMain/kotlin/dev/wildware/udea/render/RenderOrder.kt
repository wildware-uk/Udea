 ...
 * ## Determinism
 *
 * Kahn's algorithm, with the ready set kept ordered on **registration index**. That is
...
internal object RenderOrder {
    ...
    fun sort(nodeCount: Int, edges: List<OrderEdge>, describe: (Int) -> String): List<Int> {
        ...
        // Kept sorted descending, so the lowest index is the cheap removal from the end.
        ...
        while (ready.isNotEmpty()) {
            ...
            for (next in successors[node]) {
                if (--inDegree[next] == 0) insertDescending(ready, next)
            }
        }
    ...
    /** Inserts [node] into [ready], which is sorted descending and holds no duplicates. */
    private fun insertDescending(ready: ArrayList<Int>, node: Int) {
        var at = ready.size
        while (at > 0 && ready[at - 1] < node) at--
        ready.add(at, node)
    }
