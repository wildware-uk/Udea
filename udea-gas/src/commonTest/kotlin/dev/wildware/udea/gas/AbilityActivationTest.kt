...
class AbilityExecStatelessTest {
    ...
    fun `exec ids come from sorted class names - so two builds agree`() {
        ...
        assertEquals(
            forwards.idOf(RecordingExec::class.runtimeName),
            backwards.idOf(RecordingExec::class.runtimeName),
        )
    }
    ...
    fun `exec ids are the same on every target - nested classes included`() {
        ...
        val registry = AbilityExecRegistry.of(
            listOf(RecordingExec(), ExecHolderExec(), ExecHolder.InnerExec(), ChannelledExec()),
        )
        val order = listOf(
            ChannelledExec::class,
            ExecHolder.InnerExec::class,
            ExecHolderExec::class,
            RecordingExec::class,
        )
        assertEquals(
            order.indices.toList(),
            order.map { registry.idOf(it.runtimeName).index },
            "ids by class: ${order.map { it.runtimeName }}",
        )
    }
}

/** Holds a nested executor for the cross-target ordering test. */
internal object ExecHolder {
    internal class InnerExec : AbilityExec {
        override fun onActivate(context: AbilityContext) {}
    }
}

/** A top-level executor whose name starts with [ExecHolder]'s. */
internal class ExecHolderExec : AbilityExec {
    override fun onActivate(context: AbilityContext) {}
}
