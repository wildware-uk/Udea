...
| `udea-editor` | `udea.kotlin-library` | The editor window (issue #194): docked ComposeGL panels, the world in a `SceneView`, and buttons that call `editor.*` tools in-process. Debug-only, JVM | `level-editor` (D6), as a screen over the tool surface rather than a second implementation of it | `udea-render`, `udea-agent`, `udea-assets` (`api`); kotlinx-serialization-json to read tool answers | `moba:desktop`'s `editor` source set, and nothing else (`UDEA-MG-010`) |
