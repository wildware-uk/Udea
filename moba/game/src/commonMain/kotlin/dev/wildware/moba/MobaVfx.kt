 ...
 * would not boot. The **art** is authored - `moba/game/assets/effects/effects.udea.kts` declares the
...
internal class SpriteRenderSystem(
     ...
     */
    private val framesBySheet: Map<AssetId, Array<SpriteRegion>> = MobaAtlas.frames(resources)
        ...
        try {
            with(world) {
                ...
                while (index < order.size) {
                    ...
                    // The rotating overload, because the arrow has to point where it is going.
                    // The origin is the frame's centre, so a rotation spins the sprite about the
                    // entity rather than swinging it around a corner.
                    batch.draw(
                        region = frame,
                        x = position.x - width / 2f,
                        y = position.y - height / 2f + view.offsetY,
                        width = width,
                        height = height,
    ...
    private fun headingOf(entity: Entity): Float {
        ...
        // `atan2` answers in radians and the batch takes degrees. Written out rather than through
        // a platform helper, because this file compiles for every target the game has.
        return (kotlin.math.atan2(motion.vy, motion.vx) * DEGREES_PER_RADIAN)
    }
    ...
    private companion object {
         ...
         */
        const val DEGREES_PER_RADIAN: Float = 57.29578f
    }
}
