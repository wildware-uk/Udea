...
public class SimRegistry internal constructor() {
    ...
    public fun <T : SimSystem> add(
        ...
        require(registrations.none { it.type == type }) {
            "${type.runtimeName} is already registered in phase " +
    ...
    private fun edgeFor(
        ...
        val other = byType[target] ?: throw SystemOrderException(
            "${owner.name} declares $relation(${target.runtimeName}), which no module registered; " +
...
internal class SystemRegistration(
    ...
    /** Fully qualified; see [runtimeName]. */
    val name: String get() = type.runtimeName
}
