...
public sealed interface Q {
    ...
    public data object Axis8 : Q {
        ...
        override fun quantise(value: Float): Int {
            ...
            return ((t * CENTRE).roundToLong() + CENTRE).toInt()
        }
...
internal fun quantiseFixed(value: Float, min: Float, max: Float, bits: Int): Int {
    ...
    return (t * levels).roundToLong().toInt()
}
 ...
 * `[0, 1)`, but `roundToLong` rounds half up, so any `wrapped` above `1 - 0.5/2^16` — the last
...
internal fun quantiseAngle16(radians: Float): Int {
    ...
    return (wrapped * ANGLE16_LEVELS).roundToLong().toInt() and (ANGLE16_LEVELS - 1)
}
...
public fun angleDifference(a: Float, b: Float): Float {
    val d = a.toDouble() - b
    // `roundToLong` refuses NaN where `Math.round` answered zero; a NaN angle stays NaN either way.
    if (d.isNaN()) return Float.NaN
    val wrapped = d - (d / TWO_PI).roundToLong() * TWO_PI
