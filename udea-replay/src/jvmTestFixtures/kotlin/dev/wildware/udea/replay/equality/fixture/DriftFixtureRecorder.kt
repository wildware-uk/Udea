...
public object DriftFixtureRecorder {
    ...
    public fun readCheckedIn(kind: ReplayFixtureRef = DriftFixtureKind.PR): ReplayRecording {
        val bytes = checkNotNull(javaClass.getResourceAsStream(kind.resource)) {
            "${kind.resource} is not on the classpath. It is checked in under " +
                "udea-replay/src/jvmTestFixtures/resources; rebuild it with " +
     ...
     *   `udea-replay/src/jvmTestFixtures/resources/fixtures`. A parameter rather than a constant
    ...
    public const val GRADLE_TASK: String = ":udea-replay:jvmTest"
 ...
 * `:udea-replay:jvmTest -Dupdate.replay.fixtures=true`, so the two front doors cannot disagree about
