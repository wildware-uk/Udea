...
public object MobaClient {
     ...
     *
     * It goes in through [InjectedIntent] and [MobaLaunch.wireInput]'s `extra` - the **same** seam
                ...
                frame = {
                    ...
                    if (followed == null) {
                        followed = championOf(host, socket.localPeer)?.also {
                            MobaLaunch.follow(rendering, it)
                            audio.listenTo(it)
     ...
     *
     * The window's host is built by [MobaLaunch.runWithGl] as it always is, and handed straight to
                ...
                frame = { delta ->
                    ...
                    if (newMatch.poll()) {
                        MobaEntry.playerIdOrNull(host)?.let { current ->
                            MobaLaunch.follow(rendering, current)
