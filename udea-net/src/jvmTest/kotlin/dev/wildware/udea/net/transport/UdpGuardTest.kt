...
class UdpGuardTest {
    ...
    fun `a different source port does not buy a fresh bucket`() {
        ...
        repeat(HandshakeRateLimiter.DEFAULT_BURST.toInt()) {
            assertTrue(limiter.allow(InetSocketAddress(HOST, 5000 + it), now))
        }

        val fromAnotherPort = limiter.allow(InetSocketAddress(HOST, 6000), now)
    ...
    private fun address(index: Int): InetSocketAddress =
        InetSocketAddress(byteArrayOf(10, (index shr 8).toByte(), index.toByte(), 1), 4000)
