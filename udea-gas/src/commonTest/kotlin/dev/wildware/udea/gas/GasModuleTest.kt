...
class GasModuleTest {
    ...
    fun `the module registers both systems, in the phases the tick order expects`() {
        ...
        assertEquals(listOf(AbilitySystem::class.runtimeName), ability)
        assertEquals(listOf(AttributeSystem::class.runtimeName), attribute)
        assertEquals(
            listOf(GasCueForwardSystem::class.runtimeName),
            manifest.inPhase(SimPhase.Cleanup).map { it.name },
            "the cue forwarder is what connects GAS cues to GameContext.cues",
        )
        assertTrue(
            manifest.entries.indexOfFirst { it.name == AbilitySystem::class.runtimeName } <
                manifest.entries.indexOfFirst { it.name == AttributeSystem::class.runtimeName },
