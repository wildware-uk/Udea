...
- `udea-audio` → gdx, in any form. It is a designated headless module, so `UDEA-MG-002` bans
  ...
  noise is `udea-render`'s `KoolAudioDevice` (issue #221) — and, until #212 moves `moba` onto
  Kool, `moba`'s `dev.wildware.moba.audio.GdxAudioDevice` — behind this module's `AudioDevice`
