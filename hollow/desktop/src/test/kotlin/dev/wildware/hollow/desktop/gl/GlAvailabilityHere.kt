...
/**
 * Whether this JVM can create a Kool context, read from `$DISPLAY`. "Here" means *this module*: a
 * copy of `udea-render`'s `GlAvailability`, as `udea-agent-host` and `udea-editor` each keep one,
 * because that one is `internal` to its module's tests.
 *
 ...
 * `-Dudea.render.requireGl=true` turns the skip into a failure, so a job with a display cannot have
 * a backend that stopped booting hide behind a skip.
 */
