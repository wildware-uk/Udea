...
| the `dev.wildware.udea.kotlin-build-tool` stdlib pin | it reads `UdeaVersions.KOTLIN` and `check`s the running Kotlin against it, so moving the constant moved the pin with no edit here — `dev.wildware.udea.kotlin-build-tool.gradle.kts:29`, `dev.wildware.udea.kotlin-library.gradle.kts:88`. `udeaVerifyKotlinPin` runs on `check` |
