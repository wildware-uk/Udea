...
internal object ServiceIndexEmitter {
    ...
    ): FileSpec = emitElements(service, index, moduleName, property, members.map { CodeBlock.of("%T", it) })

    /**
     * [emit], for an index whose elements are expressions rather than bare declarations - a
     * [LEVEL_COMPONENTS] entry pairs a class with its serializer, see [levelComponent].
     */
    fun emitElements(
        service: ClassName,
        index: ClassName,
    ...
    private fun listInitializer(members: List<CodeBlock>): CodeBlock {
