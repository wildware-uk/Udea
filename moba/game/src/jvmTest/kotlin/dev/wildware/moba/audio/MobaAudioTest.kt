...
class MobaAudioTest {
    ...
    /** Every file the routing names is really in `:moba:game`'s asset tree. */
    @Test
    ...
    private fun assetRoot(): File {
        ...
        while (candidate != null) {
            if (File(candidate, "$ASSET_ROOT/sounds").isDirectory) {
                return File(candidate, ASSET_ROOT)
            }
            if (File(candidate, "moba/game/$ASSET_ROOT/sounds").isDirectory) {
                return File(candidate, "moba/game/$ASSET_ROOT")
            }
            candidate = candidate.parentFile
        }
        error("no moba/game/assets/sounds directory above ${File(".").canonicalFile}")
    }

    private companion object {

        /** `:moba:game`'s packed asset root, relative to that project directory. */
        const val ASSET_ROOT: String = "assets"
