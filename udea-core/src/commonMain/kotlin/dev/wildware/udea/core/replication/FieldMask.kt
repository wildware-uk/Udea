...
public object MaskOps {
    ...
    public fun cardinality(mask: FieldMask): Int = mask.bits.countOneBits()

    /** Lowest set field index, or -1 when [mask] is empty. */
    public fun lowestSetBit(mask: FieldMask): Int =
        if (mask.bits == 0L) -1 else mask.bits.countTrailingZeroBits()
    ...
    public fun nextSetBit(mask: FieldMask, from: Int): Int {
        ...
        return if (remaining == 0L) -1 else start + remaining.countTrailingZeroBits()
    }
    ...
    public fun forEachSetBit(mask: FieldMask, visitor: FieldIndexVisitor) {
        ...
        while (remaining != 0L) {
            val fieldIndex = remaining.countTrailingZeroBits()
