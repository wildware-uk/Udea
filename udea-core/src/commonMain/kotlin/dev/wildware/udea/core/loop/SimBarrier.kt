...
public class SimBarrier(initialCapacity: Int = DEFAULT_CAPACITY) {
     ...
     *
     * atomicfu's lock rather than a JVM monitor so the one piece of genuinely shared state in the
     * kernel is guarded on every target that has threads (spec section 6); on Wasm there is no
     * other thread for it to exclude.
     */
