 ...
 * bundle that arrived over the wire - which on Wasm is every bundle (issue #205).
 */
public interface BundleSource : AutoCloseable {
    ...
    public companion object {
        ...
        /**
         * A source over a file. Sections are read on demand; close it when done.
         *
         * Read through kotlinx-io's [SystemFileSystem], so this is the same code on the JVM,
         * Android, iOS and Node. A browser has no file system and fails here: a web build fetches
         * the bundle and opens the bytes instead.
         *
         * @throws FileNotFoundException when [path] does not exist - here, rather than at the
         *   first read.
         */
...
/**
 * A file read through kotlinx-io, which reads front to back and cannot seek.
 *
 * So the source keeps one open reader and its position. A range ahead of it is a skip; a range
 * behind it closes the reader and opens the file again from the start. That is the whole cost of
 * having no seek, and the order [BundleReader.open] reads in - header, table of contents, then
 * each eager section in file order - never pays it. Memory stays bounded either way: a skip
 * discards what it passes, so a streamed section is still never in memory until it is asked for.
 */
private class FileSource(private val path: Path) : BundleSource {

    override val size: Long =
        SystemFileSystem.metadataOrNull(path)?.size ?: throw FileNotFoundException("no bundle at $path")

    private var reader: Source? = null

    /** The offset [reader] reads next. Meaningless while [reader] is null. */
    private var position = 0L

    override fun read(offset: Long, length: Int): ByteArray {
        checkRange(offset, length, size)
        val current = reader?.takeIf { offset >= position } ?: reopen()
        current.skip(offset - position)
        val bytes = current.readByteArray(length)
        position = offset + length
        return bytes
    }

    private fun reopen(): Source {
        reader?.close()
        return SystemFileSystem.source(path).buffered().also {
            reader = it
            position = 0L
        }
    }
...
/** The one bounds check both sources make, so each reports an overrun in the same words. */
private fun checkRange(offset: Long, length: Int, size: Long) {
